...
from jwt_app.serializers import UserProfileSerializer, UserRegistrationSerializer, UserLoginSerializer, \
    ChangePasswordSerializer, SendPasswordResetEmailSerializer, ResetPasswordSerializer, EmailVerificationSerializer
...
from jwt_app.utils import Util
...
class UserRegisterationView(APIView):
    ...
    def post(self,request,format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            absurl = 'http://'+current_site+relative_link+"?token="+str(token.get('access'))
...
class VerifyEmail(APIView):
    ...
    @extend_schema(
        parameters=[
          EmailVerificationSerializer,  # serializer fields are converted to parameters
        ],
        # more customizations
    )
